...
### 2c. Re-check the two things `/prep-mcp` does not do

Both of these bit the Epic 17 releases and neither is covered by the command:

**Rebuild the MCP's `dist/` before trusting any n8n pre-flight.** The n8n adapter is generated by launching the built MCP server, so a stale `dist/` produces a *green* pre-flight against code that no longer exists. On 2026-07-22 the committed `dist/` was five days old.

```bash
cd pending-to-publish/<mcp> && pnpm install && pnpm run build   # then regenerate
```

**Align the lockfile with `package.json`.** `/prep-mcp` bumps `package.json#version` and never touches the lockfile, so they drift silently — `ead-factory` went out with a lockfile still declaring the previous version.
