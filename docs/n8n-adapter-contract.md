...
3. **Credential screen is an allowlist, never the MCP env set.** The n8n credential exposes **only** the fields the REST-direct `execute()` actually reads, keyed by `authStyle`:
   ...
   - `session-login-or-token` → `baseUrl` + `email` + `password` + **`sessionToken`** (user-facing products: GoCertius / EAD Enterprise Suite). The n8n user signs in as themselves: a pasted session JWT (`sessionToken`) wins; otherwise email/password, and only for **Password-type** accounts — the node calls `GET /session-info/{email}` and refuses OpenID/SSO accounts with a "paste a token" error. `sessionToken` is the one credential field with **no backing env var** (added via `EXTRA_CREDENTIAL_FIELDS`, not the env allowlist).
