...
export async function publishDockerMcpCatalog(
  ...
  // TODO: thread real source-MCP repo URL from mcp-pipeline.yaml#mcps.<name>.repo_url.
  // Today this points at the pipeline repo, which misleads reviewers about
  // where the MCP code lives. The Plan B remediation PRs opened manually from
  // HA-gdigital use the correct per-MCP source repo URL in their bodies;
  // future bot-generated PRs still get this MCP_Market_Distribution URL until
  // the follow-up PR threads the real value through. Tracked in the audit at
  // _bmad-output/research/mcp-submission-patterns-audit-2026-05-26.md.
  ...
  try {
    ...
    // 6. Open the PR upstream. Body rendered from
    // templates/store-descriptions/docker-mcp-catalog/pr-body.hbs above.
    // Off-template bodies are the primary reason 0 of our 38 marketplace
    // submissions were live as of the 2026-05-26 audit; rendering the
    // official Docker submission template (with all checkboxes ticked
    // and the credentials Google form referenced) is the fix.
    ...
    // Close stale PRs for the same MCP. The PR title carries the version
    // (`[MCP] add ead-factory v1.0.12`) so the exact-title idempotency
    // check above doesn't catch older versions — we'd accumulate one open
    // PR per release until a maintainer triages them. Search by the
    // version-less title prefix and close everything that isn't the PR we
    // just opened. Mirrors closeStaleIssues in file-marketplace-issue.ts.
    const newPrNumber = parsePrNumberFromUrl(prUrl);
    if (newPrNumber !== null) {
      const stalePrefix = `${PR_TITLE_PREFIX} ${input.mcp_name} v`;
      try {
        const staleSearch = await exec('gh', [
          'pr', 'list',
          '--repo', UPSTREAM_REPO,
          '--state', 'open',
          '--search', `${PR_TITLE_PREFIX} ${input.mcp_name} in:title`,
          '--json', 'number,title,url',
          '--limit', '30',
        ], { env: { GH_TOKEN: env.BOT_PAT } });
        if (staleSearch.exitCode === 0) {
          const stalePrs = (JSON.parse(staleSearch.stdout.trim() || '[]') as Array<{ number: number; title: string; url: string }>)
            .filter((p) => p.title.startsWith(stalePrefix) && p.number !== newPrNumber);
          for (const pr of stalePrs) {
            const closeResult = await exec('gh', [
              'pr', 'close', String(pr.number),
              '--repo', UPSTREAM_REPO,
              '--comment', `Superseded by [#${newPrNumber}](${prUrl}). Closing this older submission to keep the queue clean — please review the latest one. Generated by g-digital MCP distribution pipeline.`,
            ], { env: { GH_TOKEN: env.BOT_PAT } });
            if (closeResult.exitCode === 0) {
              log.info('target.stale_pr_closed', { ...baseEvent, pr_number: pr.number, pr_url: pr.url });
            } else {
              log.warn('target.stale_pr_close_failed', { ...baseEvent, pr_number: pr.number, stderr: closeResult.stderr.trim().slice(0, 200) });
            }
          }
        }
      } catch (err) {
        // Best-effort cleanup; never fail the publish for a stale-close failure.
        log.warn('target.stale_pr_close_failed', { ...baseEvent, reason: (err as Error).message });
      }
    }
