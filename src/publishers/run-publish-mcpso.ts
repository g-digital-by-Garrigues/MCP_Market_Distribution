...
import { publishMcpSo } from './publish-mcpso.js';
...
async function main(): Promise<number> {
  ...
  if (!mcpName || !version || !pipelineRunId) {
    process.stderr.write(
      `actions/publish-mcpso requires INPUT_MCP_NAME, INPUT_VERSION, INPUT_PIPELINE_RUN_ID env vars\n`,
    );
    return 2;
  }

  const output = await publishMcpSo({
    mcp_name: mcpName,
    version,
...
runWithCrashHandler('mcpso', main);
